...
export function BookingForm({
  ...
  const renderFieldInput = (
    ...
    return (
      <div key={field.key} className={`flex flex-col gap-1.5${field.key === "legalName" && clientType === "association" ? " lg:col-span-2" : ""}`}>
            ...
            >
              ...
              <dl className="grid gap-x-6 gap-y-3 lg:grid-cols-2 lg:gap-y-4">
                {filledAccountFieldDefinitions.map((field) => (
                  <div key={field.key} className={`min-w-0${field.key === "legalName" && clientType === "association" ? " lg:col-span-2" : ""}`}>
