...
const app = defineApp([
  ...
  route("/api/bookings", async ({ request }) => {
    ...
    try {
      ...
      let serverNetTotal = Math.max(0, serverTotalPrice - serverPromoDiscount);
      const isZeroTotal = serverNetTotal <= 0;
      if (isCashPaymentForbidden(body.paymentMethod, allowCash, serverNetTotal)) {
        return jsonResponse({ success: false, error: "Le paiement sur place n'est plus disponible, réglez en ligne par carte.", code: "cash-not-allowed" }, 400);
      }
      let bookingPaymentStatus = isZeroTotal ? "paid" : body.paymentStatus;
  ...
  route("/api/pricing", async ({ request }) => {
    ...
    try {
      ...
      const res = jsonSuccess({ grid: activeGrid, versions, minMaxByGroupType, maxAdvanceDays, openingHours, allowCash });
  ...
  route("/api/admin/bookings/:id", async ({ request, params }) => {
    ...
    if (request.method === "PUT") {
      try {
        ...
        // `loyalty_award_id` est volontairement ABSENT de cette liste : il est
        // écrit uniquement par le serveur (réclamation à la création, libération
        // sur remise manuelle ci-dessous). Exposé au client, il permettrait de
        // libérer un award tout en gardant la remise, ou d'en forger un.
        ...
        if (body.promo_discount !== undefined) {
          ...
          if (existing!.loyalty_award_id) body.loyalty_award_id = null;
  ...
  route("/api/admin/bookings/:id/send-reminder", async ({ request, params }) => {
    if (request.method !== "POST") return jsonError("Method not allowed", 405);
    ...
    try {
      ...
      const client = await getUserById(env.DB, booking.user_id);
      const eligibility = canSendBookingReminder(booking, client?.email, getParisDateISO());
      if (!eligibility.ok) return jsonError(eligibility.error, 400);
      ...
      const payload = buildBookingReminderEmailPayload({
        ...
        user: { name: client!.name, email: client!.email!, phone: client!.phone },
        payments,
        whenPhrase: eligibility.whenPhrase,
      });
      const sent = await sendBookingReminderEmail(env.RESEND_API_KEY, payload);
      if (!sent.success) {
        return jsonError("Échec de l'envoi de l'email de rappel", 502);
      }

      await addAuditLog(env.DB, "booking", params.id, "send-reminder", {
        to: client!.email,
        whenPhrase: eligibility.whenPhrase,
        remainingDue: payload.reminder?.remainingDue ?? 0,
      }, request.headers.get("X-Admin-User-Id") || "admin");

      return jsonSuccess({ sent: true, to: client!.email });
    } catch (error) {
      console.error("POST /api/admin/bookings/:id/send-reminder error:", error);
      return jsonError(error instanceof Error ? error.message : "Failed to send reminder", 500);
    }
  }),
    ...
    if (request.method === "PUT") {
      try {
        ...
        if (["loyalty_enabled", "loyalty_discount_type", "loyalty_discount_value", "loyalty_threshold"].some((key) => Object.prototype.hasOwnProperty.call(rawBody, key))) {
          ...
          const loyalty = validateLoyaltySettings(rawBody);
          if (!loyalty.ok) return jsonError(loyalty.error, 400);
  ...
  route("/api/client/loyalty", async ({ request }) => {
    if (request.method !== "GET") return jsonError("Method not allowed", 405);
    const token = getClientSessionToken(request);
